...
void player_move_forwards(Player *self, double dt, float direction){
	...
	moveable_object_move_by(&self->moveable, vec3_scale(VEC3_UNIT_Z, -dt * speed * direction));
}
...
void player_strafe(Player *self, double dt, float direction){
	...
	moveable_object_move_by(&self->moveable, vec3_scale(VEC3_UNIT_X, dt * speed * direction));
}
...
void player_handle_inputs(Player *self, double dt, Inputs inputs){
	...
	self->moveable.direction = vec3_sub(mouse_world, self->moveable.position);
	vec3_normalize(&self->moveable.direction);
