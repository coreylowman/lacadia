...
static void permafrost_particle_init(Particle *p, Vec3 position, float duration){
...
Permafrost *permafrost_new(GameWorld *world, GameObject *target, float dmg, float slow_pct, float duration){
    ...
    self->particle_system = particle_system_new(world, target->position, "assets/frost_particle", 64, duration, duration * 0.2);
    particle_system_set_particle_init(self->particle_system, permafrost_particle_init);
...
static void permafrost_on_apply(Effect *self, AffectableComponent *affectable){
    ...
    if (permafrost != NULL){
        ...
        // free the effect we were trying to add, since we just modified the permafrost
...
static void permafrost_on_end(Effect *self){
    // undo the slow
    self->amount_affected -= affectable_component_quicken(self->container, self->amount_affected);
}
