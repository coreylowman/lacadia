...
Frost *frost_new(GameWorld *world, GameObject *target, float slow_pct_per_degree, float duration){
    ...
    self->particle_system = particle_system_new(world, target->position, "assets/frost_particle", 8, duration, duration * 0.4);
...
void frost_increase_degree(Effect *effect){
	...
	effect->amount_affected += affectable_component_slow(self->base_effect.container, amt);
}
