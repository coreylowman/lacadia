...
int obb_intersects(Obb a, Obb b) {
  ...
  t = vec3_from_3f(vec3_dot(t, a.axis[0]), vec3_dot(t, a.axis[1]),
                   vec3_dot(t, a.axis[2]));
  ...
  for (i = 0; i < 3; i++) {
    ra = a.radius.data[0] * absR[0].data[i] +
         a.radius.data[1] * absR[1].data[i] +
         a.radius.data[2] * absR[2].data[i];
    rb = b.radius.data[i];
    if (fabs(t.x * R[0].data[i] + t.y * R[1].data[i] + t.z * R[2].data[i]) >
        ra + rb)
      return 0;
  }
