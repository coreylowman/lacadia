...
ParticleSystem *particle_system_new(GameWorld *world, Vec3 position,
                                    const char *asset_name, int num_particles,
                                    float duration, float particle_duration) {
  ParticleSystem *self = malloc(sizeof(*self));
  self->base_object =
      game_object_init(world, "particle_system", particle_system_update,
  ...
  for (i = 0; i < num_particles; i++)
    self->particle_init(&self->particles[i], position, self->particle_duration);

  self->renderable = renderable_component_init(&self->base_object, asset_name,
                                               world->renderer);
...
void particle_system_double_particles(ParticleSystem *self) {
  ...
  for (; i < self->num_particles; i++) {
    self->particle_init(&self->particles[i],
                        self->follow_target ? self->follow_target->position
                                            : self->base_object.position,
                        self->particle_duration);
  }
}

void particle_system_set_scale_over_duration(ParticleSystem *self,
                                             int scale_over_duration) {
  self->scale_over_duration = scale_over_duration;
}

void particle_system_set_particle_init(
    ParticleSystem *self,
    void (*particle_init_arg)(Particle *p, Vec3 position, float duration)) {
...
void particle_system_render(GameObject *obj, Renderer *renderer) {
  ...
  for (i = 0; i < self->num_particles; i++) {
    ...
    if (self->scale_over_duration) {
      mat4_scale(&model_matrix,
                 self->particles[i].duration / self->particle_duration);
    }
...
void particle_system_set_follow_target(ParticleSystem *self,
