...
static int load_png_data(const char *filename, TextureAsset *texture) {
  ...
  if (!error)
    error = lodepng_decode32(&texture->data, &texture->width, &texture->height,
                             png, pngsize);
  if (error) {
  ...
  if (directory != NULL) {
    while ((entry = readdir(directory)) != NULL) {
      ...
      case DT_DIR:
        ...
        result =
            load_textures(self, dir_path, dir_name_len + strlen(entry->d_name));
  ...
  if (directory != NULL) {
    while ((entry = readdir(directory)) != NULL) {
      ...
      case DT_DIR:
        ...
        result =
            load_models(self, dir_path, dir_name_len + strlen(entry->d_name));
        dir_path[dir_name_len] = 0;
        if (result) {
          goto end;
        }
