...
Level *level_new(GameWorld *world){
    ...
    self->walls[1] = wall_new(world, (Vec3){ .data = { dims.x * -5, 0.0, dims.z * -5} }, VEC3_UNIT_X, 10);
    self->walls[2] = wall_new(world, (Vec3){ .data = { dims.x * -5, 0.0, dims.z * -4} }, VEC3_UNIT_Z, 9);
    self->walls[3] = wall_new(world, (Vec3){ .data = { dims.x * 5, 0.0, dims.z * -4} }, VEC3_UNIT_Z, 9);
    int i;
    for(i = 0;i < self->num_walls;i++){
        game_world_add_wall(world, self->walls[0], i);
    }
    return self;
}
