...
static void dragons_breath_apply(GameWorld *world, GameObject *user, Enemy *enemy) {
    ...
    if (degrees_between <= 45) {
        ...
        affectable_component_affect(&enemy->affectable, soulburn_new(world, &enemy->base_object, 3, 0.5, 1.5));
    }
}
...
void dragons_breath_use(GameWorld *world, GameObject *user){
    ...
    game_world_apply_to_enemies(world, user, radius, dragons_breath_apply);

    ParticleSystem *ps = particle_system_new(world, user->position, "assets/burn_particle", 64, 0.0, 0.75);
