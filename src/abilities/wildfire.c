...
static void wildfire_apply(GameWorld *world, GameObject *user, GameObject *enemy);
static void wildfire_spread(GameWorld *world, GameObject *origin, GameObject *enemy);
...
static void wildfire_on_collide(GameObject *self, GameObject *other){
    if (other->type == GAME_OBJECT_TYPE_ENEMY) {
        ...
        if (enemy->affectable.effects[EFFECT_TYPE_BURN] != NULL){
            ...
            affectable_component_affect(&enemy->affectable, (Effect *)burn_new(self->world, &enemy->base_object, 1, 4));
        }
...
static void wildfire_spread(GameWorld *world, GameObject *origin, GameObject *target){
    ...
    if(enemy->affectable.effects[EFFECT_TYPE_BURN] == NULL){
        ...
        game_world_add_object(world, (GameObject *)wildfire_spread_new(world, origin, (GameObject *)enemy));
    }
}
...
static void wildfire_apply(GameWorld *world, GameObject *user, GameObject *target){
    ...
    if(enemy->affectable.effects[EFFECT_TYPE_BURN] != NULL){
        ...
        game_world_add_object(world, (GameObject *)wildfire_new(world, user, (GameObject *)enemy));
    }
}
